...
object Apis {
    ...
    fun paperVersions(): List<String> = try {
        ...
        (0 until a.length()).map { a.getString(it) }
            ...
            .sortedByDescending { v: String -> versionRank(v).joinToString(".") { it.toString().padStart(6, '0') } }
    ...
    fun forgeVersions(): List<String> = try {
        ...
        o.keys().asSequence().filter { it.endsWith("-recommended") || it.endsWith("-latest") }
            ...
            .filter { it.matches(Regex("\\d+\\.\\d+(\\.\\d+)?")) }
            .sortedByDescending { v: String -> versionRank(v).joinToString(".") { it.toString().padStart(6, '0') } }.toList()
    } catch (_: Exception) { emptyList() }
